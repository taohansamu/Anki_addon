...
class ModelMappingDialog(QDialog):
	...
	def __createMemriseModel(self, course, pool):
		...
		name = u"Memrise - {} - {}".format(course.title, pool.name)
		m = mm.new(name)
...
class MemriseImportDialog(QDialog):
	def __init__(self, memriseService):
		...
		label = QLabel("Names of levels must contain at least:")
		...
		minimalLevelTagWidthTooltip = "digits (so e.g. '3' will result in Level001, '4' in 'Level0001' etc."
